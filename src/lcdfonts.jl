...
function lcdstring!(string_cell::Cell, str::String, pixelsize, pixelspacing; meta::Meta=GDSMeta(0,0), scripting = false, linelimit = typemax(Int), verbose = false)
    ...
    superscript = +1
    waitforend = false
    existing_chars = Dict{Char, Devices.Cells.CellReference}()
    for s in str
        if subscript == 0
            offset = -0.3
        ...
        elseif s == '}' && scripting
            ...
            if !haskey(existing_chars, s)
                verbose && println("Character '", s, "' not found. Adding to CellReference dictionary.")
                if haskey(lcd, s)
                    s_cell = (typeof(string_cell))(uniquename("lcd"))
                    drawlcdcell!(s_cell, lcd[s], pixelsize, pixelspacing, meta)
